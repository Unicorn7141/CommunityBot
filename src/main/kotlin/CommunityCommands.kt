...
class CommunityCommands : Extension() {
    ...
    class ShopAddRole : Arguments() {
        ...
        val name by string("name", "The role's name (wrap with \" if spaces are needed)")
    ...
    override suspend fun setup() {
        ...
        publicUserCommand {
            ...
            action {
                val target = targetUsers.toList()[0].asMember(getGuild()?.id ?: error("Invalid Guild"))
            ...
            action {
                ...
                val giftedCommunityMember = ser.community.find { it.id == gifted.id.asString }
                    ?: when {
                        gifted.isNullOrBot() -> {
                            respond {
                                content = "You can't gift bots and people who are no longer in this server"
                            }
        ...
        chatCommand(::GiftArgs) {
            ...
            action {
                ...
                if (amount > gifter.money) {
                    ...
                    val gifted_ = ser.community.find { it.id == gifted.id.asString }
                        ?: error("Cannot find user: ${gifted.id.asString} in guild ${ser.id}")
                    val gifterIndex = ser.community.indexOf(gifter)
                    val giftedIndex = ser.community.indexOf(gifted_)
                    ser.community[giftedIndex].money += amount
                    ser.community[gifterIndex].money -= amount
                    message.reply {
                        allowedMentions()
                        embed {
                            ...
                            description =
                                "${gifted.mention} you were gifted $amount ⍟ by ${message.getAuthorAsMember()!!.mention}"
                            color = Color(50, 205, 50)
                        }
                    }
                }
